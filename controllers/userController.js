...
module.exports = {
  ...
  resetPassword: async (req, res) => {
    ...
    const user = await UserModel.findOne({
      $and: [{ email: userEmail.email }, { status: false }],
    });
    if (!user) {
      return res.redirect("/signin");
    } else {
  ...
  updateProfile: async (req, res) => {
    ...
    let details = await UserModel.findOneAndUpdate(
      { _id: req.params.id },
  ...
  filterOne: async (req, res) => {
    const products = await ProductModel.find({
      $and: [{ status: false }, { price: { $lte: 99, $gte: 0 } }],
    });
    const category = await CategoryModel.find({ status: false });
    if (req.session.userLogin) {
  ...
  filterTwo: async (req, res) => {
    const products = await ProductModel.find({
      $and: [{ status: false }, { price: { $lte: 499, $gte: 100 } }],
    });
    const category = await CategoryModel.find({ status: false });
    if (req.session.userLogin) {
      res.render("user/filter", {
  ...
  filterThree: async (req, res) => {
    const products = await ProductModel.find({
      $and: [{ status: false }, { price: { $lte: 999, $gte: 500 } }],
    });
    const category = await CategoryModel.find({ status: false });
    if (req.session.userLogin) {
      res.render("user/filter", {
  ...
  filterFour: async (req, res) => {
    const products = await ProductModel.find({
      $and: [{ status: false }, { price: { $lte: 1999, $gte: 1000 } }],
    });
    const category = await CategoryModel.find({ status: false });
    if (req.session.userLogin) {
      res.render("user/filter", {
  ...
  filterFive: async (req, res) => {
    const products = await ProductModel.find({
      $and: [{ status: false }, { price: { $lte: 3999, $gte: 2000 } }],
    });
    const category = await CategoryModel.find({ status: false });
    if (req.session.userLogin) {
      res.render("user/filter", {
  ...
  cartPage: async (req, res) => {
    ...
    let CartData = await CartModel.findOne({ userId: userId }).populate(
      "items.productId"
    );
    if (Cart) {
  ...
  cartIncrement: async (req, res) => {
    ...
    let cart = cartData[0];
    let items = cart.items;
    let itemIndex = items.findIndex((p) => p.productId == prodId);
    let product = items[itemIndex];
  ...
  coupon: async (req, res) => {
    ...
    const couponData = await CouponModel.find({
      $and: [{ couponCode: coupon }, { status: false }],
    });
  ...
  trackOrder: async (req, res) => {
    ...
    // order.map((it)=>{
    //     let {id,address,userId,...items}=it
    //     console.log(items,'spread');
    //   items.forEach((aj)=>{
    //     console.log(aj,'sufiyan');
    //   })
    // })
    // order.forEach((itm)=>{
    ...
    //     console.log(itm.items,'aj');
    // })
    // console.log('najwaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', order);
    let orders = [];
    let product = [];
    let products = [];
    let tempp;
    for (let i = 0; i < order.length; i++) {
      product[i] = order[i].items[0].productId.valueOf();
      tempp = await ProductModel.find({ _id: product[i] });
      products[i] = tempp[0];
      orders[i] = order[i].items[0];
    
    }
    // var tracker = await TrackModel({
    ...
    //     totalProduct:orders[i].totalProduct,
